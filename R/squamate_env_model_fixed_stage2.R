...
env_vae_mod <- nn_module("ENV_VAE",
                         ...
                         loss_function = function(reconstruction, input, mean, log_var, mean_spec, 
                                                  lambda = 1,
                                                  alpha = 1) {
                         ...
                         forward = function(x, s) {
                           ...
                           list(self$decoder(z = z, s = s), x, s, means, log_vars)
                         }
...
for (epoch in 1:num_epochs) {
  ...
  coro::loop(for (b in train_dl) {
    ...
    c(loss, reconstruction_loss, kl_loss) %<-% env_vae$loss_function(reconstruction, input, means, log_vars, mean_spec)
...
torch_save(env_vae, "data/env_vae_1_trained_fixed_16d_stage2.to")
